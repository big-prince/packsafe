    ...
    try {
      ...
      for (const [keyString, indexNames] of indexKeys) {
        if (indexNames.length > 1) {
          logger.warn(
            `Found duplicate indexes for ${keyString}: ${indexNames.join(', ')}`
          );
      ...
      logger.error(
        `Error cleaning indexes for collection ${collectionName}:`,
        error
      );
    }
  }
    ...
    try {
      ...
      if (existingIndex) {
        ...
        if (JSON.stringify(existingIndex.key) === JSON.stringify(keys)) {
          logger.info(
            `Index ${indexName} already exists and matches specification`
          );
          return;
        } else {
          // Drop and recreate if different
          logger.info(
            `Recreating index ${indexName} due to specification change`
          );
        ...
        for (const index of indexes) {
          if (index.name !== '_id_') {
            try {
              ...
              logger.info(
                `Dropped index ${index.name} from ${collection.name}`
              );
            } catch (error) {
